//@version=6
indicator("CS - Fundamentals & Daily SMAs", overlay = true, max_bars_back = 300)
...
group_sma = "Moving Average Lines & Timeframe"
bool   showSmas = input.bool(true, "Show SMA Lines", group = group_sma)
...
// =============================================================================
// 2. TECHNICALS: TIMEFRAME RESOLUTION & SMAS
// =============================================================================
string targetTf = useChartTf ? timeframe.period : "D"

// Calculate SMAs and liquidity natively on the target timeframe using historical security calls
float sma10 = request.security(syminfo.tickerid, targetTf, ta.sma(close, 10), barmerge.gaps_off, barmerge.lookahead_off)
float sma21 = request.security(syminfo.tickerid, targetTf, ta.sma(close, 21), barmerge.gaps_off, barmerge.lookahead_off)
float sma50 = request.security(syminfo.tickerid, targetTf, ta.sma(close, 50), barmerge.gaps_off, barmerge.lookahead_off)
float sma200 = request.security(syminfo.tickerid, targetTf, ta.sma(close, 200), barmerge.gaps_off, barmerge.lookahead_off)

plot(showSmas and show10Sma ? sma10 : na, title = "10 SMA", color = color.yellow, linewidth = 1)
plot(showSmas and show21Sma ? sma21 : na, title = "21 SMA", color = color.fuchsia, linewidth = 1)
plot(showSmas and show50Sma ? sma50 : na, title = "50 SMA", color = color.orange, linewidth = 2)
plot(showSmas and show200Sma ? sma200 : na, title = "200 SMA", color = color.navy, linewidth = 2)

// 50-Day Supply & Demand Liquidity Averages
float avg50DollarVol = request.security(syminfo.tickerid, targetTf, ta.sma(close * volume, 50), barmerge.gaps_off, barmerge.lookahead_off)
float avg50ShareVol = request.security(syminfo.tickerid, targetTf, ta.sma(volume, 50), barmerge.gaps_off, barmerge.lookahead_off)
